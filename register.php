...
if( isset($_POST['submit']) ){
        ...
        $nowhatsapp     = stripslashes($_POST['nowhatsapp']);
        $nowhatsapp     = mysqli_real_escape_string($con, $nowhatsapp);
        ...
        if(!empty(trim($name)) && !empty(trim($username)) && !empty(trim($email))&& !empty(trim($nowhatsapp)) && !empty(trim($password)) && !empty(trim($repass))){
            ...
            if($password == $repass){
                ...
                if( cek_nama($name,$con) == 0 ){
                    ...
                    $query = "INSERT INTO users (username,name,email, password ) VALUES ('$username','$nama','$email','$nowhatsapp','$pass')";
